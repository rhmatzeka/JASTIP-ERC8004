...
export default function RoleEntryCards() {
  const { role, setRole } = useDemoProfile();
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      {Object.values(DEMO_PROFILES).map((profile) => {
        ...
        return (
          <article key={profile.role} className={`panel p-5 ${active ? "border-ocean ring-2 ring-ocean/15" : ""}`}>
            <div className="mb-4 flex items-start justify-between gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-lg bg-cloud text-ocean">
                <Icon size={20} />
              </span>
              {active ? <span className="rounded-full bg-mint/10 px-3 py-1 text-xs font-bold text-emerald-700">Active</span> : null}
            </div>
            ...
            <Link href={meta.href} className="btn-primary mt-5 w-full" onClick={() => setRole(profile.role)}>
              {meta.cta}
              <ArrowRight size={16} />
            </Link>
          </article>
